...
/**
 * Main plugin file for the Mason WordPress customizations plugin for the instance: gmuwptheme
 */

/**
 * Plugin Name:       Mason WordPress: Customizations Plugin: gmuwptheme
 * Author:            Mason Web Administration
 * Plugin URI:        https://github.com/mason-webmaster/gmuw-wordpress-plugin-mason-customizations-gmuwptheme
...
'https://github.com/mason-webmaster/gmuw-wordpress-plugin-mason-customizations-gmuwptheme/',
__FILE__,
'gmuw-wordpress-plugin-mason-customizations-gmuwptheme'
);
...
add_filter ('manage_website_posts_columns', 'gmuw_gmuwptheme_add_columns_website');
function gmuw_gmuwptheme_add_columns_website ($columns) {
...
add_action ('manage_website_posts_custom_column', 'gmuw_gmuwptheme_website_custom_column', 10, 2);
function gmuw_gmuwptheme_website_custom_column ($column, $post_id) {
 switch ($column) {
   case 'post_id':
    echo $post_id;
    break;
   ...
   case 'wordpress_theme_live':
      if (get_post_meta($post_id, 'website_status', true)!='deleted') {
        echo gmuj_gmuwptheme_get_live_website_theme($post_id);
      }
     break;
...
function gmuj_gmuwptheme_get_live_website_theme($post_id){
        ...
        if (gettype($mason_site_check_in_theme_info_response_json)!='object'){
          ...
          if ($mason_site_check_in_theme_info_response_json->data->status==404){
            ...
            if (get_post_meta($post_id, 'wordpress_theme', true ) != $mason_site_check_in_theme_info_response_json->theme_name.' ('.$mason_site_check_in_theme_info_response_json->theme_version.')') {
              $return_value.='<p>';
              $return_value.=gmuj_gmuwptheme_update_theme_link($post_id,$mason_site_check_in_theme_info_response_json->theme_name,$mason_site_check_in_theme_info_response_json->theme_version);
              $return_value.='</p>';
            }
...
function gmuj_gmuwptheme_update_theme_link($post_id,$theme_name,$theme_version){
  //Display theme update link

  //Return value
  return "<a class='button button-small' href='admin.php?page=gmuj_gmuwptheme&action=update_theme&post_id=". $post_id ."&theme_name=". urlencode($theme_name) ."&theme_version=". $theme_version ."'>Update Post</a>";
