 ...
 */
function gmuj_gmuwptheme_hosting_domain_url($web_host, $environment_name) {
 ...
 */
function gmuj_gmuwptheme_total_websites() {
 ...
 */
function gmuj_gmuwptheme_websites_using_theme() {
 ...
 */
function gmuj_gmuwptheme_website_action_update_theme() {
	...
	echo "<div id='gmuj_gmuwptheme_update_theme_output' class='updated fade'>";
 ...
 */
function gmuj_gmuwptheme_display_settings_page() {
	...
	if ($_GET["action"]=='update_theme') { 
		gmuj_gmuwptheme_website_action_update_theme();
	}
	...
	//Get stats
		// Total number of websites
			$websites_all_count = gmuj_gmuwptheme_total_websites();
		// Websites using official theme
			$websites_theme_count = gmuj_gmuwptheme_websites_using_theme();
		// Theme percentage
	...
	foreach ( $websites_all as $post ) {
		...
		echo '<td><a href="'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'">'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'</a></td>';
		...
		if ($post->website_status=='deleted') {
			...
			echo '<td>' . '<a href="'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'/wp-json/gmuj-sci/theme-info">theme info</a><br /><a href="'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'/wp-json/gmuj-sci/most-recent-modifications">modifications</a><br /><a href="'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'/wp-json/gmuj-mmi/mason-site-info">site info</a></td>';
		}
		...
		if ($post->website_status=='deleted') {
			...
			echo '<td><a href="'.gmuj_gmuwptheme_hosting_domain_url($post->web_host,$post->environment_name).'/wp-admin/" target="_blank" title="WordPress login"><img style="width:25px;" src="'.plugin_dir_url( __DIR__ ).'images/logo-wordpress.png'.'" /></a></td>';
